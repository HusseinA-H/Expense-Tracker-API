...
async def test_create_category(
    client: AsyncClient, auth_headers: dict, uow: SQLAlchemyUnitOfWork, test_user
):
    payload = {"name": "Subscriptions", "icon": "card", "color": "#123456"}
...
async def test_update_category(
    ...
    payload = {"name": "Custom Cat", "icon": "icon", "color": "#112233"}
    ...
    # Update it
    update_payload = {"name": "Updated Cat", "color": "#778899"}
    update_response = await client.patch(
        f"/api/v1/categories/{cat_id}", json=update_payload, headers=auth_headers
    )
...
async def test_update_system_category_forbidden(
    client: AsyncClient, auth_headers: dict, uow: SQLAlchemyUnitOfWork
):
    # Find a system category
    async with uow:
        categories = await uow.categories.get_all_accessible(
            uuid.uuid4()
        )  # any uuid works since system cats are fetched
...
async def test_delete_category(
    ...
    payload = {"name": "To Delete", "icon": "bin", "color": "#990000"}
    create_response = await client.post(
        "/api/v1/categories", json=payload, headers=auth_headers
    )
    cat_id = create_response.json()["id"]
...
async def test_delete_system_category_forbidden(
    ...
    response = await client.delete(
        f"/api/v1/categories/{system_cat_id}", headers=auth_headers
    )
    assert response.status_code == 403
    assert response.json()["error"]["code"] == "FORBIDDEN"
...
async def test_category_isolation(
    ...
    payload = {"name": "User 1 Private", "icon": "lock", "color": "#111111"}
    create_response = await client.post(
        "/api/v1/categories", json=payload, headers=auth_headers
    )
    cat_id = create_response.json()["id"]
    ...
    delete_response = await client.delete(
        f"/api/v1/categories/{cat_id}", headers=other_auth_headers
    )
