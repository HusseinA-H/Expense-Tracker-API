...
async def test_transaction_crud_audit_logs(
    ...
    create_res = await client.post(
        "/api/v1/transactions", json=tx_payload, headers=auth_headers
    )
    ...
    audit_res = await client.get(
        f"/api/v1/audit/logs?action=CREATE&entity_type=transaction&entity_id={tx_id}",
    ...
    update_payload = {"amount": 35.00, "description": "Updated lunch audit test"}
    update_res = await client.patch(
        f"/api/v1/transactions/{tx_id}", json=update_payload, headers=auth_headers
    )
    assert update_res.status_code == 200

    # 5. Query audit logs as Admin to assert the UPDATE log exists
    audit_res = await client.get(
        f"/api/v1/audit/logs?action=UPDATE&entity_type=transaction&entity_id={tx_id}",
        headers=admin_headers,
    )
    assert audit_res.status_code == 200
    audit_data = audit_res.json()
    ...
    audit_res = await client.get(
        f"/api/v1/audit/logs?action=DELETE&entity_type=transaction&entity_id={tx_id}",
        headers=admin_headers,
    )
    assert audit_res.status_code == 200
    audit_data = audit_res.json()
...
async def test_auth_and_user_actions_audit_logs(
    ...
    audit_res = await client.get(
        f"/api/v1/audit/logs?action=CREATE&entity_type=user&entity_id={user_id}",
        headers=admin_headers,
    )
    assert audit_res.status_code == 200
    ...
    # 2. Login
    login_payload = {"email": "audit_user@example.com", "password": "Password123!"}
    ...
    audit_res = await client.get(
        f"/api/v1/audit/logs?action=LOGIN&entity_type=user&entity_id={user_id}",
        headers=admin_headers,
    )
    assert audit_res.status_code == 200
    ...
    # 3. Change password
    pw_payload = {"current_password": "Password123!", "new_password": "NewPassword123!"}
    pw_res = await client.put(
        "/api/v1/users/me/password", json=pw_payload, headers=user_headers
    )
    assert pw_res.status_code == 204

    # Assert password change audit log exists
    audit_res = await client.get(
        f"/api/v1/audit/logs?action=PASSWORD_CHANGE&entity_type=user&entity_id={user_id}",
        headers=admin_headers,
    )
    assert audit_res.status_code == 200
    ...
    audit_res = await client.get(
        f"/api/v1/audit/logs?action=DELETE&entity_type=user&entity_id={user_id}",
        headers=admin_headers,
    )
    assert audit_res.status_code == 200
