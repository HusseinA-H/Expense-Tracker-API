...
async def test_create_duplicate_budget(
    client: AsyncClient, auth_headers: dict, uow: SQLAlchemyUnitOfWork
):
    async with uow:
        categories = await uow.categories.get_all_accessible(uuid.uuid4())
...
async def test_list_budgets(
    ...
    response = await client.get(
        "/api/v1/budgets?month=6&year=2026", headers=auth_headers
    )
    ...
    response_empty = await client.get(
        "/api/v1/budgets?month=7&year=2026", headers=auth_headers
    )
...
async def test_budget_isolation(
    client: AsyncClient,
    auth_headers: dict,
    ...
    list_res = await client.get(
        "/api/v1/budgets?month=6&year=2026", headers=other_auth_headers
    )
    ...
    delete_res = await client.delete(
        f"/api/v1/budgets/{budget_id}", headers=other_auth_headers
    )
...
async def test_budget_summary(
    ...
    summary_res = await client.get(
        "/api/v1/budgets/summary?month=6&year=2026", headers=auth_headers
    )
